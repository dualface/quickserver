...
function ChatRoom:ctor(app)
    ...
    if app then 
        if app.requestType == "websockets" then
            ...
            self.reply = Err(ERR_CHAT_OPERATION_FAILED, "ChatRoom: this plugin can't be access by http")
            return self.reply
        end
    end
...
function ChatRoom:_RemoveChatRecords(count)
...
function ChatRoom:Broadcast(data)
